...
## 7. 動作確認（clone と push ができるか）

ツールがそろったか、実際に試します。講師から案内されたリポジトリ、または自分のリポジトリで行います。

```bash
# 1. ダウンロードできるか（<owner>/<repo> は実際の値に置き換える）
...
# 3. 認証が通るか（リモート情報を取得するだけ。変更はしない）
git fetch
```

`git fetch` がエラーなく終われば、認証は通っています。
（実際の push は、次のページ [01. ローカル開発サイクル](01-local-flow.md) で安全な手順とともに行います。）

> 🔑 SSH を選んだ場合は、clone の URL を `git@github.com:<owner>/<repo>.git` にします。
...
- 次へ: [01. ローカル開発サイクル（clone → 編集 → commit → push）](01-local-flow.md)
