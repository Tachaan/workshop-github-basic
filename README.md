...
> Template から自分専用の練習Repoを作り、簡易アプリのバグ修正を通して **Issue / Branch / Commit / Pull Request / Review / Merge / 履歴確認** の基本を体験します。
...
- 開発環境（**Git / VSCode / GitHub認証**）、または **Codespaces** を使えること
  - ローカルを使う場合は [00. 環境構築](onboarding/00-setup.md) で準備（約30〜45分）。

**本日のゴール**

- GitHubを使った基本の開発の流れを理解すること

Git と GitHub を初めて使う参加者が、Falling Blocks アプリのバグ修正を題材に、Repository作成からMerge後の履歴確認までを1本のストーリーで体験し、
「なぜブランチを切るのか」「なぜ Pull Request を作るのか」「レビュー後の変更をどう追うのか」を理解するためのワークショップです。

> 📝 練習Repoは [`Tachaan/github-basic-practice`](https://github.com/Tachaan/github-basic-practice) の **Use this template** から作ります。既定ブランチは `main` です。
...
| Repositoryをどう始めるか分からない | Templateから自分の練習Repoを作り、Owner / Public / mainを確認する |
...
| チーム開発の流れを想像できない | 同じRepoでIssueからMerge、log / blameまでを1サイクルで体験する |

> 💡 **キーメッセージ**: GitHubでの基本の開発は「目的を残し、小さく分け、見える形で相談し、合意してから取り込む」こと。
...
├── templates/     Issue / PR 記入例と練習Template Repoの管理用ファイル
├── participants/  任意の追加練習用ディレクトリ
├── scripts/       ローカルプレビュー起動スクリプト
└── .github/       GitHub上で使うIssue / Pull Requestテンプレート
```

> 🔑 **設計のキモ**: 参加者ごとのTemplate Repoを1本のストーリーで使い続け、Markdown・通知・開発・Review・履歴検索を必要な手順の中で体験します。ローカルが難しければCodespacesへ切り替えられます。

## 📝 表記について

- GitHub の画面に出る名前は、教材内でも **Issue / Branch / Commit / Pull Request / Review / Merge** のように英語表記を基本にします。
- 説明文では、必要に応じて日本語で意味を補足します。
- 練習Templateの既定ブランチは `main` です。別の名前が表示された場合は、指定Templateから作ったRepoか確認してください。

---

## 🚀 クイックスタート

### 0. ローカルでワークショップサイトを見る

ローカルプレビューは、講師や教材編集者が Markdown 教材を共通ナビゲーションとテーマで確認するための手順です。
既存のスライド形式のハンズオンと、受講者が修正するアプリも同じサーバーから表示できます。
...
既存のスライド形式のハンズオンは http://127.0.0.1:8000/handson/ で確認できます。
...
| 立場 | まず読むもの |
| --- | --- |
| 受講者 | [ハンズオン手順](handson/README.md) → [基本編: Template Repo作成から履歴確認まで](handson/01-github-flow-web.md) |
...
受講者は [`handson/README.md`](handson/README.md) から開始し、練習Template [`Tachaan/github-basic-practice`](https://github.com/Tachaan/github-basic-practice) を使います。

基本編では、Falling Blocks アプリの底判定バグを題材に以下を体験します。

1. Templateから `github-basic-practice-<github-id>` を作り、ReviewペアをCollaboratorへ招待する
2. MarkdownでIssueを書き、Label / Assignee / @メンションを設定する
3. Repoをclone、またはCodespacesで開く
4. `fix-falling-blocks-floor-<github-id>` Branchを作り、`game.js`を修正する
5. Commit / Pushし、`Closes #番号` とReviewer付きのPull Requestを作る
6. PR URLを交換してペアReviewし、Repo所有者がMergeする
7. Issue Close、Branch削除、`git log` / `git blame` / 相互リンクを確認する
...
| **M2** 開発の流れ | ブランチ運用の型 | main から分岐し、PR で戻す流れを理解する |
| **M3-1** Repo / Issue | 作業開始とコミュニケーション | Template、Collaborator、Markdown、Label、Assignee |
| **M3-2** Branch / Commit | VSCodeで開発 | clone / Codespaces、Branch、修正、Commit / Push |
| **M3-3** PR / Review | ペアで確認 | Closes、Reviewer指定、Files changed、Review |
| **M3-4** Merge / 履歴 | 取り込み後を追う | Issue Close、Branch削除、log、blame、相互リンク |
| **発展（任意）** | トラブル対応 | コンフリクト・やり直し（+20分） |
| **M4** まとめ | 次の一歩 | 今日の流れを言語化し、続編につなげる |
...
基本編では、コードの修正を自分の PC の **VSCode** で行います。ローカル構築が難しい場合は **Codespaces** を選べます。
**導入手順は [onboarding/00-setup.md](onboarding/00-setup.md) でガイド**します（約30〜45分）。
...
- GitHub Codespaces（ローカル環境の代替。無料枠内で利用可能）
